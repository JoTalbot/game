...
(function (G) {
  ...
  G.World.prototype.update = function (dt, player, dna, fx, game) {
    ...
    for (var w = this.wounds.length - 1; w >= 0; w--) {
      ...
      if (wd < player.r + u.r) {
        // Укус остаётся сильным (22/с): смягчение до 9 сломало долг
        // памяти — игрок переставал убегать, существа всегда были рядом
        // и голод не дозревал ни у кого. Рана обязана гнать с места.
        // Лечится не сила укуса, а НЕИЗБЕЖНОСТЬ: рана была быстрее
        // игрока (86 против 83) и бессмертна. Теперь медленнее — от неё
        // можно уйти, — и истлевает за полторы минуты.
