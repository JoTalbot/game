...
(function (G) {
  ...
  G.World.prototype.update = function (dt, player, dna, fx, game) {
    ...
    for (var i = this.nodes.length - 1; i >= 0; i--) {
      ...
      if (n.state === "alive" && n.care > 0.62) {
        ...
        // первого. Полный распад теперь ~3 минуты полного забвения.
        if (n.roots > 0 && n.care < 0.28) n.roots = Math.max(0, n.roots - dt * 0.006);
  ...
  G.World.prototype.metamorphose = function (player, dna) {
    ...
    for (var i = 0; i < this.nodes.length; i++) {
      ...
      if (this.anchors.indexOf(n.id) >= 0 || n.roots >= 0.6) {
        n.x *= 0.15;
        n.y *= 0.15;
        n.hp = 1;
        keep.push(n);
