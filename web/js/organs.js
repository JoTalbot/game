...
(function (G) {
  ...
  G.Organs = {
    ...
    maybeBoss: function (game) {
      ...
      // HP рос от w.lost — а это счётчик ЗА ВСЮ ЖИЗНЬ (не обнуляется на
      // метаморфозе). На седьмом берегу босс собирался уже со ~135 hp: его
      // нельзя было убить, он лишь гонялся за человеком и съедал две трети
      // сессии (отчёт 2.3: босс 21560 силы, «падала до 0»). Босс должен
      // расти от брошенного, но в разумных пределах — не в губку. Вклад
      // lost капится: бой остаётся боем, а не получасовым отгрызанием.
    ...
    updateBoss: function (game, dt) {
      ...
      boss.x += boss.vx * dt;
      boss.y += boss.vy * dt;
      // Босс — существо, а не декорация. Пока он жив и недалеко, он подаёт
      // голос сам — редко (не чаще раза в ~45 с) и не перебивая Игру.
      // Раньше долгий бой был тишиной: человек, чью сессию босс съедал на
      // две трети, слышал лишь «собралось из брошенного» и потом пустоту —
      // и справедливо назвал игру «слишком молчу». Голос босса наполняет
      // тишину и учит, не впадая в туториал: с ним говорят пульсом.
      if (d < 520 && G.Voice && !G.Voice.visible &&
          (!boss._talkAt || game.time - boss._talkAt > 45)) {
        boss._talkAt = game.time;
        G.Voice.say("bossSpeak");
      }
      if (d < boss.r + p.r + 6) {
        // Голод с твоим лицом: тень кусает ровно настолько, насколько
        // агрессивен ты сам. Мирный игрок чувствует лишь лёгкий холод,
        // воин — настоящий укус. Раньше 6/с было для всех одинаково, и
        // мирный садовник-сбой всё равно садился в ноль под боссом
        // (отчёты 2.12/2.13: «падала до 0»). Теперь босс — зеркало, а не
        // налог: не провоцируешь — он почти не трогает.
    ...
    resolveChord: function (game) {
      ...
      // Аккорд больше НЕ ставит якорь.
      //
      // Якорь — самый дорогой жест игры: им человек говорит «это не
      // отпускай». А созвучие складывается само, от ходьбы по тонам, и
      // раздавало якоря молча: замер — 14 якорей из 14 пришли отсюда,
      // ни одного от удержания. Отчёт человека: 18 узлов, ДЕВЯТЬ якорей
      // при пределе три. Он их не ставил — они ставились сами.
      //
      // Ту же болезнь я чинил в 0.4.50 и вылечил половину: убрал якорь
      // с рождения, а этот вход не нашёл. Награда, которую выдают без
      // просьбы, обесценивает жест целиком.
      //
      // Созвучие вместо якоря греет: узлы, попавшие в такт, оживают —
      // это его собственный дар, не чужой.
      // Созвучие греет ВСЁ, что попало в такт, а не один узел. Прежде
      // оно якорило ближайший — берег держался на этом, и, отняв якорь,
      // я обеднил его с 21 до 18 живых узлов за час. Тепло по кругу —
      // честная замена: аккорд не «сохраняет навсегда», он оживляет то,
      // что рядом, и человек всё равно должен возвращаться.
      for (var wi = 0; wi < game.world.nodes.length; wi++) {
        var wn = game.world.nodes[wi];
        if (wn.state !== "alive" || wn.dead) continue;
        // Радиус 220 выверен замером: при 150 берег беднеет и долг
        // памяти перестаёт дозревать вовсе (пик 1.00 при пороге 1.2,
        // ноль исходов) — узлы тонут раньше, чем существо успевает
        // привязаться. Шире — теплица, уже — пустошь.
        if (G.dist(p.x, p.y, wn.x, wn.y) < 220) {
          wn.care = 1;
          if (wn.hp < 1) wn.hp = Math.min(1, wn.hp + 0.35);
        }
      }
