...
(function (G) {
  ...
  var MAX_EVENTS = 24, MAX_CAUSES = 48;
  var EVENTS = [
  ...
  function fresh(){return {version:1,act:1,turns:0,conflict:"",conflictScore:0,events:[],causes:[],endings:[],route:"",active:false,complete:false};}
  function load(){var s=fresh();try{var raw=G.Save&&G.Save.get?G.Save.get(KEY):null,p=raw?JSON.parse(raw):null;if(p&&typeof p==="object"){s.act=Math.max(1,Number(p.act)||1);s.turns=Math.max(0,Number(p.turns)||0);s.conflict=String(p.conflict||"");s.conflictScore=Number(p.conflictScore)||0;s.events=Array.isArray(p.events)?p.events.slice(-MAX_EVENTS):[];s.causes=Array.isArray(p.causes)?p.causes.slice(-MAX_CAUSES):[];s.endings=Array.isArray(p.endings)?p.endings.slice(-4):[];s.route=String(p.route||"");s.active=!!p.active;s.complete=!!p.complete;}}catch(e){}return s;}
  ...
  function route(sig){
    // Current behavior wins over inherited bond/return history when it is a clear severance signal.
    // Otherwise relationship/history can select the quieter bonding and stewardship routes.
    if(sig.sever||sig.avoid)return "severing";
    if(sig.bond&&sig.return)return "bonding";
    if(sig.care&&sig.return)return "steward";
    if(sig.still&&sig.care)return "watching";
    return "enduring";
  }
  function applyPhysical(game,e,r){var w=game.world,p=game.player,nodes=w.nodes||[],node=null;for(var i=0;i<nodes.length;i++){if(!nodes[i]||nodes[i].dead)continue;var d=G.dist?G.dist(p.x,p.y,nodes[i].x,nodes[i].y):9999;if(d<850){node=nodes[i];break;}}if(!node&&nodes[0])node=nodes[0];if(node){node.act2Trace=true;node.memory=true;node.historyV4=Math.min(1,Number(node.historyV4||0)+0.025);if(r==="steward")node.care=Math.min(1,Number(node.care||0)+0.035);if(r==="bonding")node.bondTrace=true;if(r==="severing")node.scars=Number(node.scars||0)+1;}if(w.scatter)w.scatter(p.x,p.y,r==="severing"?1:2,r==="bonding"?300:420);if(G.V4History&&G.V4History.state){var hs=G.V4History.state();hs.lastCause=e.causeId||hs.lastCause;}}
  function cause(s,type,source,target,data){var id="v4c-"+type+"-"+source+"-"+target+"-"+s.causes.length;s.causes.push({id:id,type:type,source:source,target:target,data:data||{}});while(s.causes.length>MAX_CAUSES)s.causes.shift();return id;}
  ...
  function addEnding(s,game,r,ending){if(s.endings.indexOf(ending)>=0)return false;s.endings.push(ending);var ec=cause(s,"ending",r,ending,{events:s.events.length,conflict:s.conflict});applyPhysical(game,{causeId:ec,place:"threshold"},ending==="keep"?"steward":"severing");return true;}
  function observe(dt,game){if(!game||game.state!=="play")return;var s=state(),rs=G.ReleaseSystems&&G.ReleaseSystems.state?G.ReleaseSystems.state():null;if(rs&&rs.act<2)return;s.active=true;s.act=2;s.turns++;var sig=signalMap(game),r=route(sig);s.route=r;if(!s.conflict)s.conflict=sig.bond?"memory-vs-distance":sig.care?"care-vs-change":"return-vs-severance";s.conflictScore+=(sig.bond?0.4:0)+(sig.care?0.3:0)+(sig.wound?0.25:0)+(sig.memory?0.15:0);if(s.turns%30===0&&s.events.length<EVENTS.length){var picked=pickEvent(s,sig,r);if(picked){var cid=cause(s,"event",picked.id,picked.place,{route:r,signals:picked.need});var recorded={id:picked.id,place:picked.place,route:r,causeId:cid};s.events.push(recorded);applyPhysical(game,recorded,r);}}if(!s.complete&&s.events.length>=6){var desired=(r==="bonding"||r==="steward")?"keep":"let-go";if(s.endings.length===0){addEnding(s,game,r,desired);}else if(s.endings.length===1&&s.endings[0]!==desired&&s.turns%60===0){addEnding(s,game,r,desired);}}if(s.endings.length>=2)s.complete=true;save(s);}
