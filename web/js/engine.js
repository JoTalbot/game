...
(function (G) {
  ...
  G.Game.prototype.onDown = function () {
    ...
    // 0.28 было быстрее пальца на стекле (Android double-tap ~0.30).
    // Отчёты 2.3–2.32: пульсов 0. Не туториал — окно жеста.
    if (dtap < 0.40 && this.state === "play") {
  ...
  G.Game.prototype._gaze = function (dt) {
    ...
    // Порог срыва — 126, а не 112. Все отчёты с телефона показывали одно и
    // то же: «палец уезжал на медиана 113–118, максимум 120 (порог 112)».
    // Срыв «палец ушёл» случался РОВНО на границе порога — человек почти
    // удержал, но естественное движение пальца чуть переваливало 112, и
    // взгляд рвался за 0.1 с до рождения (1.35 с). Подняли над максимумом
    // уезжания с запасом: спокойная рука держит, дрогнувшая — рвёт, и
    // отменить жест, уведя палец, по-прежнему можно (126 точек — это
    // треть экрана, дальше человек и не поведёт при отмене).
    if (slip > 126) {
