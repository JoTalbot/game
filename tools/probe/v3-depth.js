...
function seedSignals(){
  ...
  G.Save.set("igra.spatial-memory.v1",JSON.stringify({version:1,visits:3,returns:1,places:[{x:0,y:0,kind:"spark",roots:0.7,care:0.9,life:1,visits:2}],last:null}));
  G.Relationships.resetCache();G.WorldMemory.resetCache();G.SpatialMemory.resetCache();G.V3Depth.resetCache();
}
function findNode(game, key){
  for(var i=0;i<game.world.nodes.length;i++) if(game.world.nodes[i] && game.world.nodes[i][key]) return game.world.nodes[i];
  return null;
}
...
G.V3Depth.observe(1,game);var rareNode=findNode(game,"personalEcho");ok(G.V3Depth.profile().rare.length===1&&!!rareNode,"V3-021 создаёт персональный редкий след из нескольких систем");G.V3Depth.observe(1,game);ok(G.V3Depth.profile().rare.length===1,"V3-021 не спамит одинаковым моментом");
clean();seedSignals();game=H.makeWorld(G,8811);node=game.world.nodes[0];node.x=game.player.x+20;node.y=game.player.y+20;node.state="alive";node.dead=false;G.Life.resetCache();G.Act.resetCache();G.Life.arc().initialized=true;G.Life.arc().life=1;G.Act.state().phase=2;G.V3Depth.observe(1,game);G.V3Depth.observe(1,game);var p=G.V3Depth.profile();ok(p.places.length===1&&p.places[0].visits===1,"V3-024 одно посещение места не считается каждый кадр");ok(!!findNode(game,"placeMemory"),"V3-024 материализует историю места в самом берегу");
