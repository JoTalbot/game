...
const g=H.makeWorld(G,7000);g.state="play";if(G.ReleaseSystems&&G.ReleaseSystems.state)G.ReleaseSystems.state().act=3;for(const k of G.TRAITS)g.dna.set(k,k==="empathy"?0.95:0.01);g.world.wounds=[{x:0,y:0}];G.V6Body.reset();G.V6Body.observe(g);ok(g.player.bodyForm&&g.player.bodyDepth>=2,"body identity is exposed to active gameplay context");
