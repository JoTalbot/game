...
function run(seed) {
  ...
  const myth = G.V10Myth.create(seed);
  ...
  for (let i = 0; i < 10000; i++) {
    ...
    if (i % 97 === 0) myth.touch({ type, region, dominant: i % 2 ? "curiosity" : "harmony" });
  }
  return { world: plain(world.snapshot()), myth: plain(myth.snapshot()), sim: plain(G.V19Simulation.snapshot(world)) };
}
...
assert(a.myth.signals.length <= 12, "myth signals remain bounded");
