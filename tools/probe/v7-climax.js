...
const g=H.makeWorld(G,7000);g.state="play";const rs=G.ReleaseSystems.state();rs.act=3;rs.actTurns=0;rs.ecology=0.78;for(const k of G.TRAITS)g.dna.set(k,k==="harmony"?0.9:0.02);for(let i=0;i<240;i++){g.time=3000+i;G.ReleaseSystems.observe(1,g);}
...
G.V7Climax.reset();G.ReleaseSystems.state().act=3;G.ReleaseSystems.state().ecology=0.82;for(const k of G.TRAITS)g.dna.set(k,k==="empathy"?0.9:0.02);G.V7Climax.observe(g);G.V7Climax.choose("release",g);const release=G.V7Climax.profile();ok(release.finale==="release"&&release.post===true,"release leaves a post-finale state");const releaseState=release.worldState;
G.V7Climax.reset();G.ReleaseSystems.state().act=3;G.ReleaseSystems.state().ecology=0.82;for(const k of G.TRAITS)g.dna.set(k,k==="aggression"?0.9:0.02);G.V7Climax.choose("become",g);const become=G.V7Climax.profile();ok(become.finale==="become"&&become.post===true,"become leaves a post-finale state");ok(releaseState!==become.worldState,"finale routes remain distinguishable");ok(g.finalWorldState===become.worldState&&g.postFinale===true,"active game receives post-finale context");
