...
function run(seed) {
  ...
  const before = JSON.stringify(G.V19Simulation.snapshot(world));
  const processed = G.V19Simulation.tick(world, 0.25);
  const after = G.V19Simulation.snapshot(world);
...
assert.notStrictEqual(a.before, JSON.stringify(a.after), "deep simulation mutates live state");
...
assert.deepStrictEqual(JSON.parse(JSON.stringify(G.V19Simulation.snapshot(restored))), a.after, "simulation snapshot restores exactly");
