...
const malformed = { simulationV19: { tick: "bad", budget: "bad", accumulator: "bad", cursor: "bad", historyCursor: "bad", migration: "bad", memory: null, causalEvents: 7 } };
const safe = G.V19Simulation.ensure(malformed);
assert(Number.isFinite(safe.budget) && safe.budget >= 1 && safe.budget <= 8, "malformed budget is sanitized");
assert(Array.isArray(safe.migration) && Array.isArray(safe.memory) && Array.isArray(safe.causalEvents), "malformed collections are sanitized");
