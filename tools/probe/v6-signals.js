...
function game(trait,depth,scars){const g=H.makeWorld(G,7000);g.state="play";if(G.ReleaseSystems&&G.ReleaseSystems.state)G.ReleaseSystems.state().act=Math.max(1,depth+1);for(const k of G.TRAITS)g.dna.values[k]=k===trait?0.95:0.01;g.bodyIdentity={form:{harmony:"tender",empathy:"echo",contemplation:"still",curiosity:"seeking",aggression:"scar",chaos:"shard"}[trait],depth,scars};return g;}
