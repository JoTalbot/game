...
assert(P && P.currentSchema === 4, "actual persistence module loads at schema 4");
var sim = ctx2.IGRA.V19Simulation.ensure(ctx2.IGRA.V9World.create(777));
ctx2.IGRA.V19Simulation.tick(sim === null ? null : ctx2.IGRA.V9World.create(777), 0.25);
...
game.world.v9v25.world = game.world.v9v25.world;
...
assert.strictEqual(migrated.schema, 4, "legacy integration state migrates to schema 4");
assert(Array.isArray(migrated.replay), "migration creates replay array");
var old = Object.assign({}, migrated, { schema: 2 });
assert.strictEqual(P.migrate(old).schema, 4, "schema 2 migrates to schema 4");
