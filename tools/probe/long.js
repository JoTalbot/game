...
// Палец ходит по берегу, а не лежит вечно в одной точке.
//
// Раньше стенд ставил касание и держал его неподвижно две секунды,
// каждый раз в ТУ ЖЕ мировую точку. Пока захват решался только в
// onDown, это было безобидно. Но игра научилась ловить взгляд у
// замершего пальца (0.4.49) — и робот превратился в вечный источник
// тепла: он бесконечно грел один узел, care держался на 1.00, прилив
// не мог его забрать, берег переползал предел до 34 узлов. Проверка
// краснела на болезни, которой у живого человека нет: тот отпускает
// палец и уходит. Двигаем касание — как рука.
function poke(g, f) {
  if (g.input.down && f % 30 === 0) {
    g.input.x += 40;
    g.input.y += 25;
    var wm = g.screenToWorld(g.input.x, g.input.y);
    g.input.wx = wm.x; g.input.wy = wm.y;
  }
  if (f % 180 === 0) {
    var n = null;
