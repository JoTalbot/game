...
(function () {
  ...
  ok(/порог срыва 112/.test(gtext), "видно, насколько уезжал палец",
...
// ——— босс ест силу ———
// «Сила ушла на» знала взгляд, раны и пульсы — но не босса, а он ест
// 38/с, больше любого укуса раны. Разбивка врала ровно в бою, где человек
// теряет больше всего, и «падала до» застывала на последнем замеренном
// от взгляда. Каждый канал оттока обязан быть посчитан.
group("отчёт: босс тоже ест силу");
(function () {
  var game = H.makeWorld(G, 7);
  G.Report.reset();
  // босс прямо на игроке — кусает 28/с (было 38, снижено по отчёту 0.4.80: босс съедал 78% всей силы)
  game.world.boss = {
    x: game.player.x, y: game.player.y, r: 28,
    phase: 0, stun: 0, weak: 0, vx: 0, vy: 0, lunge: 0,
    hp: 10, maxHp: 10, parts: []
  };
  for (var i = 0; i < 60; i++) G.Organs.updateBoss(game, 1 / 60);
  ok(G.Report.drain.boss > 20, "босс попадает в разбивку расхода силы",
     "съедено " + Math.round(G.Report.drain.boss) + " за секунду (реально 28)");
  ok(G.Report.lowest <= 80, "падение силы под боссом видно в отчёте",
