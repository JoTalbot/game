...
// в бою. Босс обязан попадать в отчёт. А после 2.13 он кусает 6/с (было
// 28/с — самый быстрый отток, обнулял энергию за секунды): психологическая
// угроза, а не налог. Обе правды — в отчёте и в мире.
...
(function () {
  ...
  ok(G.Report.drain.boss > 3, "босс попадает в разбивку расхода силы",
     "съедено " + Math.round(G.Report.drain.boss) + " за секунду (6/с)");
  ok(G.Report.lowest >= 80, "босс не обнуляет — пугает, а не калечит",
     "падала до " + G.Report.lowest + " (при 28/с было бы 72)");
