...
(function (G) {
  ...
  // Свет — главное, чем этот мир говорит. Но радиальный градиент строится
  // заново на каждый вызов, а вызовов в кадре 234: треть всей работы холста
  // уходит на свечение. G.Quality.glow (lang.js) для слабого телефона уже
  // считался — и не читался НИКЕМ: обещание было объявлено и не исполнено.
  //
  // Гасить свет нельзя — это и есть красота. Поэтому на слабом телефоне
  // рисуем то же пятно двумя плоскими кругами: ядро и ореол. Мягкость
  // теряется, свет остаётся. Три операции вместо градиента.
  // Свет — главное, чем этот мир говорит, и гасить его нельзя.
  //
  // Но радиальный градиент строится заново на каждый вызов, а вызовов в
  // кадре 234 — треть всей работы холста уходит на свечение. При этом
  // G.Quality.glow (lang.js) для слабого телефона считался и не читался
  // НИКЕМ: обещание было объявлено и не исполнено.
  //
  // Первым лечением я заменил градиент двумя плоскими кругами — и замер
  // это опроверг: два круга это шесть операций против четырёх у градиента.
  // Стало дороже. Поэтому лечу не цену свечения, а их число: у каждого
  // вызова есть чин. Свет смысла (узлы, существа, игрок, зов) горит
  // всегда. Свет украшения (звёзды памяти, пылинки) на слабом телефоне
  // гаснет — но сами звёзды остаются, у них своё сплошное ядро.
  function glow(ctx, x, y, r, rgb, a, decor) {
  ...
  G.Renderer = {
    ...
    draw: function (ctx, game) {
      ...
      for (var m = 0; m < stars.length; m++) {
        ...
        // На слабом телефоне ореол гаснет, но само зерно звезды ниже
        // рисуется всегда: небо забвения остаётся населённым.
        glow(ctx, sx, sy, sky ? 22 : 10, st.c, 0.22 * ta, true);
    ...
    drawBloom: function (ctx, cam, b, t) {
      ...
      // сад: до сорока цветов разом, каждый со своим ореолом — украшение
      glow(ctx, p.x, p.y, r * 4, c, 0.12, true);
      ctx.save();
    ...
    drawBeing: function (ctx, cam, b, t) {
      ...
      // Преданность видна глазом: спутник (bond прошёл порог) носит
      // тонкое тёплое кольцо — как якорь у узла. До этого преданность
      // читалась только по следованию и свечению, которое росло и у
      // просто знакомых. Кольцо — отметка «это уже не чужое»: видно,
      // кто твой, даже в темноте, ещё до того, как спутник заговорит.
      if (b.bond > 0.6 && !b.isYesterday) {
        ctx.strokeStyle = G.rgb(255, 214, 170, 0.30 + 0.18 * Math.sin(t * 2 + b.phase));
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(p.x, p.y, r * 1.25, 0, G.TAU);
        ctx.stroke();
      }
