...
(function (G) {
  ...
  G.Being = function (x, y, dnaHint) {
    ...
    this.babyKey = 0;
    this.name = "";
    // Память событий существа: remember() пишет сюда «ударили/тронули/
    // смотрели», speakBeing() читает её длину, чтобы чередовать реплики.
    // Раньше массив инициализировался ТОЛЬКО в birthBeing — а load()
    // поднимает существо через new G.Being, где его не было. Вернувшийся
    // человек касался существа (remember) или держал на нём взгляд
    // (speakBeing) — и кадр падал на `b.memory.length` у undefined:
    // `Cannot read properties of undefined`. Это не потеря памяти, а крах.
    // Инициализируем здесь, чтобы был жив ЛЮБОЙ путь создания.
    this.memory = [];
    this.lastNear = 0;
  };
  ...
  G.World.prototype.update = function (dt, player, dna, fx, game) {
    ...
    for (var j = this.beings.length - 1; j >= 0; j--) {
      ...
      if (d < 56) {
        ...
        var hungerThresh = b._woundedRescued ? 0.75 : 0.61;
    ...
    return {
      ...
      nodes: this.nodes.map(function (n) {
        return {
          ...
          state: n.state,
          care: n.care,
          // обещанная природа. Узел может быть ещё неоформленным, но уже
          // задуман: метка первого берега, гнездо после зова, «эхо» органа
          // эмпатии. Без этого поля сейв забывал замысел, и обещанное
          // после зова гнездо нужной природы вырастало из жеста, а не из
          // награды за дорогу.
          hint: n.hint,
