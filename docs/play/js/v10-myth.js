...
(function (G) {
  ...
  function ensure(world) {
    ...
    if (!world.mythV10 || typeof world.mythV10 !== "object") {
      world.mythV10 = { version: 1, generation: 0, lives: [], memories: [], signals: {}, current: null };
    }
    var s = world.mythV10;
    s.version = 1;
  ...
  function absorb(world, finale, profile, region) {
    ...
    s.lives.push(memory);
    s.memories.push({ generation: s.generation, type: "finale", value: memory.finale, dominant: dominant, region: memory.region });
  ...
  function start(world) {
    ...
    var prior = last || { finale: "unknown", dominant: "unknown", freedom: 0.4, bond: 0.4, change: 0.45, region: "r0" };
    var conditions = {
      generation: s.generation,
      echo: prior.finale,
  ...
  function touch(world, region, kind, amount) {
    ...
    var key = cleanId(kind || "unknown", "unknown");
    ...
    if (s.signals[key] >= 0.7) {
      s.memories.push({ generation: s.generation, type: "signal", value: key, region: cleanId(region, "r0") });
      s.memories = s.memories.slice(-MAX_MEMORY);
    }
  ...
  function snapshot(world) {
    var s = ensure(world); if (!s) return null;
    return JSON.parse(JSON.stringify({ version: 1, generation: s.generation, lives: s.lives.slice(-MAX_LIVES), memories: s.memories.slice(-MAX_MEMORY), signals: s.signals, current: s.current }));
  }
  ...
  G.V10Myth = { ensure: ensure, absorb: absorb, start: start, touch: touch, rare: rare, snapshot: snapshot, restore: restore, constants: { maxMemory: MAX_MEMORY, maxLives: MAX_LIVES, maxSignals: MAX_SIGNALS } };
})(IGRA);
