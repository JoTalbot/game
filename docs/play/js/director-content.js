...
(function (G) {
  ...
  var POOLS = {
    secondAct: [
      ...
      { id: "witness-returns", phase: 4, need: function (c) { return c.returns >= 3 && c.beings >= 2; }, apply: function (g) { var b = nearestBeing(g, 320); if (!b) return false; b.memoryMark = true; b.returnEcho = true; b.bond = Math.min(1, (Number(b.bond) || 0) + 0.05); b.memory = Array.isArray(b.memory) ? b.memory : []; b.memory.push({ t: Date.now(), ev: "returned" }); if (b.memory.length > 6) b.memory.shift(); return true; } },
      { id: "law-bends-around-care", phase: 4, need: function (c) { return c.ecology >= 0.65; }, apply: function (g) { var n = nearest(g, 360); if (!n) return false; n.lawTrace = "care-reinforces-return"; n.care = Math.min(1, (Number(n.care) || 0) + 0.12); n.roots = Math.max(Number(n.roots) || 0, 0.82); scatter(g, n, 2, 210); return true; } },
      ...
      { id: "old-wound-grows", phase: 5, need: function (c) { return c.wounds >= 1 && c.skins >= 2; }, apply: function (g) { var n = nearest(g, 240); if (!n) return false; n.scarMemory = true; n.actTrace = true; n.care = Math.max(Number(n.care) || 0, 0.7); n.scars = (Number(n.scars) || 0) + 1; return true; } },
      { id: "scar-teaches-route", phase: 5, need: function (c) { return c.wounds >= 1 && c.returns >= 4; }, apply: function (g) { var n = nearest(g, 300); if (!n) return false; n.scarMemory = true; n.returnEcho = true; n.routeTrace = true; n.care = Math.max(Number(n.care) || 0, 0.62); scatter(g, n, 1, 260); return true; } },
      ...
      { id: "echo-keeps-memory", phase: 5, need: function (c) { return c.trust >= 0.5 && c.memories >= 2 && c.beings >= 2; }, apply: function (g) { var b = nearestBeing(g, 360); if (!b) return false; b.memoryMark = true; b.echoMemory = Math.max(Number(b.echoMemory) || 0, 1); b.relationship = b.relationship || {}; b.relationship.memories = Math.max(Number(b.relationship.memories) || 0, 1); return true; } }
    ]
  };
