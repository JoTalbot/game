...
(function (G) {
  ...
  function step(game,dt){var s=ensure(game);if(!s||!s.world)return;var d=clamp(finite(dt,0),0,0.25),before=s.world.history.length,start=typeof performance!=="undefined"&&performance.now?performance.now():0;s.clock+=d;var regionId=regionFor(game),action=actualAction(game,regionId,s),region=s.world.region(regionId);s.lastRegion=regionId;s.lastAction=action||{type:"idle",region:regionId,amount:0};
    if(action){s.metrics.actions++;if(action.type==="visit")s.metrics.moves++;if(action.type==="care"||action.type==="harm")s.metrics.gazes++;if(action.type==="pulse")s.metrics.pulses++;if(s.world.observe)s.world.observe(d,action);}else{s.metrics.idleSteps++;if(s.world.observe)s.world.observe(d,null);}
    var c=counters(game);s._counters=c;s._px=finite(game.player&&game.player.x,0);s._py=finite(game.player&&game.player.y,0);touchPersonality(game,!!action&&!!game.gazeTarget);social(game,action);knowledge(game,action);if(action||s.clock-s._lastDirector>=2){director(game,action||{type:"idle",region:regionId,amount:0});s._lastDirector=s.clock;}presentationAudio(game,region,action);experiment(game,action,before);s.stepCount++;s.eventCount=s.world.history.length;
