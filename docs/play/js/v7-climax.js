...
(function (G) {
  ...
  var DOMINANT_ROUTE = { harmony:"steward", empathy:"bonding", contemplation:"watching", curiosity:"seeking", aggression:"severing", chaos:"severing" };
  function fresh(){return {version:1,tick:0,phase:"approach",chain:[],worldState:"balanced",finale:"",post:false,generation:0,lastCause:""};}
  function load(){var s=fresh();try{var r=G.Save&&G.Save.get?G.Save.get(KEY):null,p=r?JSON.parse(r):null;if(p&&typeof p==="object")Object.keys(s).forEach(function(k){if(p[k]!=null)s[k]=p[k];});}catch(e){}if(!Array.isArray(s.chain))s.chain=[];s.chain=s.chain.filter(function(v){return typeof v==="object"&&v;}).slice(-MAX);s.tick=Math.max(0,Number(s.tick)||0);s.phase=typeof s.phase==="string"?s.phase:"approach";s.worldState=typeof s.worldState==="string"?s.worldState:"balanced";return s;}
  ...
  function trajectory(game){if(!game)return null;try{if(G.Trajectory&&typeof G.Trajectory.build==="function"){var t=G.Trajectory.build(game);if(t&&t.dominant)return String(t.dominant).toLowerCase();}}catch(e){}return null;}
  function route(game){var dominant=trajectory(game);if(dominant&&DOMINANT_ROUTE[dominant])return DOMINANT_ROUTE[dominant];var p=G.V6Signals&&G.V6Signals.profile?G.V6Signals.profile(game):null;var form=p?p.form:"shoreborn";return form||"shoreborn";}
  function causeId(s,game,label){var rs=G.ReleaseSystems&&G.ReleaseSystems.state?G.ReleaseSystems.state():null;var base=(rs&&rs.lastCause)||"v7";var text=base+":"+label+":"+route(game)+":"+(game&&game.time||0)+":"+s.tick;var h=2166136261;for(var i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619);}return "v7-"+(h>>>0).toString(36);}
  function worldState(game){var rs=G.ReleaseSystems&&G.ReleaseSystems.state?G.ReleaseSystems.state():null;var e=rs?Number(rs.ecology||0.5):0.5;var r=route(game);if(r==="severing")return e<=0.45?"hollow":"scarred";if(r==="bonding"||r==="steward")return e>=0.67?"flourishing":"balanced";if(e>=0.82)return "flourishing";if(e<=0.20)return "hollow";return "balanced";}
  ...
  function choose(choice,game){var s=state(),rs=G.ReleaseSystems&&G.ReleaseSystems.state?G.ReleaseSystems.state():null,nl=rs&&rs.nextLife?rs.nextLife:null;s.finale=choice==="become"?"become":"release";s.worldState=worldState(game);s.post=true;s.generation=Math.max(s.generation,Number(nl&&nl.generation||0));var cid=causeId(s,game,"finale-"+s.finale);s.lastCause=cid;s.chain.push({tick:s.tick,phase:"finale",state:s.worldState,route:route(game),causeId:cid});if(s.chain.length>MAX)s.chain.shift();save(s);if(game){game.finalWorldState=s.worldState;game.postFinale=true;}}
