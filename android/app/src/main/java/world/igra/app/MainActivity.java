...
public class MainActivity extends Activity {
    ...
    void fitToScreen() {
        ...
        final int contentH = webView.getContentHeight();
        final float density = getResources().getDisplayMetrics().density;

        float zoom = 1f;
        if (contentH > 8) {
            float contentPhys = contentH * density;
            if (contentPhys < viewH * 0.82f) {
                zoom = viewH / (float) contentH;
            }
        }
        ...
        final String js =
                ...
                + "window.__IGRA_PX_W=" + viewW + ";"
                + "window.__IGRA_PX_H=" + viewH + ";"
                + "window.__IGRA_ZOOM=" + z + ";"
                + "if(window.IGRA&&IGRA.app&&IGRA.app.resize)IGRA.app.resize();"
                + "else window.dispatchEvent(new Event('resize'));"
                + "})()";
        webView.evaluateJavascript(js, null);
        if (z > 1.05f) {
            webView.setInitialScale(Math.round(100f * z));
        }
    }
    ...
    private static class AssetClient extends WebViewClient {
        ...
        public void onPageFinished(WebView view, String url) {
            view.postDelayed(new Runnable() {
                public void run() { host.fitToScreen(); }
            }, 60);
            view.postDelayed(new Runnable() {
                public void run() { host.fitToScreen(); }
            }, 280);
